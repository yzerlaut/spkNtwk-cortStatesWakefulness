...
# %% [markdown]
# # Response to Gaussian blob appearances
...
# %%
Episodes_GB = physion.dataviz.show_data.EpisodeResponse(episodes_GB)
...
# %%
ROI_SUMMARIES = [episodes_GB.compute_summary_data(dict(interval_pre=[-episodes_GB.visual_stim.protocol['presentation-interstim-period'],0], 
                                                       interval_post=[0,episodes_GB.visual_stim.protocol['presentation-duration']],
                                                       test='wilcoxon', 
                                                       positive=True),
...
# %%
stim_keys, stim_indices = ['center-time', 'radius'], [1,0]
responsive_rois = find_responsive_rois(episodes_GB, stim_keys, stim_indices, 
                                      ROI_SUMMARIES)

fig, AX = physion.analysis.behavioral_modulation.plot_resp_dependency(Episodes_GB,
...
# %%
stim_keys = [key for key in Episodes_GB.varied_parameters if key!='repeat']
set_of_stim_indices = list(itertools.product(*[range(len(Episodes_GB.varied_parameters[key])) for key in stim_keys]))
...
analysis_dir = os.path.join(os.path.expanduser('~'), 'Desktop', 'GB-analysis')
...
for f, subject, pid in zip(DATASET['files'][start_index:],
    ...
    for i, stim_indices in enumerate(set_of_stim_indices):
        ...
        fig.savefig(os.path.join(file_folder, 'GB-%i.png' % (i+1)))
        plt.close(fig)
